<?php /*a:2:{s:102:"/Users/ding/Documents/GSB 3/4.15/label-00896-GSB-0415/frontend-admin/app/view/admin/product/index.html";i:1776232727;s:109:"/Users/ding/Documents/GSB 3/4.15/label-00896-GSB-0415/frontend-admin/app/view/admin/../admin/layout/base.html";i:1776232727;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
    ...
    <title>产品管理 - <?php echo htmlentities((string) $site_name); ?></title>
...
<div class="filter-card">
    <form class="row g-3 align-items-end" method="get">
        <div class="col-auto">
            <input type="text" class="form-control" name="keyword" value="<?php echo htmlentities((string) (isset($keyword) && ($keyword !== '')?$keyword:'')); ?>" placeholder="搜索产品名称">
        </div>
        <div class="col-auto">
            <select class="form-select" name="status">
                <option value="">全部状态</option>
                <option value="1" <?php if(isset($status) && $status === '1'): ?>selected<?php endif; ?>>上架</option>
                <option value="0" <?php if(isset($status) && $status === '0'): ?>selected<?php endif; ?>>下架</option>
            </select>
        </div>
        <div class="col-auto ms-auto">
            <button type="submit" class="btn btn-primary"><i class="bi bi-search me-1"></i>搜索</button>
            <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#editModal" onclick="openAdd()"><i class="bi bi-plus me-1"></i>新增</button>
        </div>
    </form>
</div>
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?php echo htmlentities((string) mb_substr($item['name'],0,30,'utf-8')); ?></td>
                    <td>¥<?php echo htmlentities((string) number_format($item['price'],2)); ?></td>
                    <td>
                        <?php if($item['status'] == 1): ?>
                        <span class="status-tag success">上架</span>
                        <?php else: ?>
                        <span class="status-tag danger">下架</span>
                        <?php endif; ?>
                    </td>
                    ...
                    <td class="action-btns">
                        <button class="btn btn-sm btn-outline-primary" onclick="openEdit(<?php echo htmlentities((string) $item['id']); ?>)">编辑</button>
                        <button class="btn btn-sm btn-outline-<?php if($item['status'] == 1): ?>warning<?php else: ?>success<?php endif; ?>" onclick="toggleStatus('/admin/product/status/<?php echo htmlentities((string) $item['id']); ?>', $(this))"><?php if($item['status'] == 1): ?>下架<?php else: ?>上架<?php endif; ?></button>
                        <button class="btn btn-sm btn-outline-danger" onclick="confirmDelete('/admin/product/delete/<?php echo htmlentities((string) $item['id']); ?>', $(this).closest('tr'))">删除</button>
                    </td>
                </tr>
        ...
        <div class="d-flex align-items-center">
            ...
            <ul class="pagination pagination-sm mb-0">
                ...
                <li class="page-item <?php if($currentPage <= 1): ?>disabled<?php endif; ?>">
                    <a class="page-link" href="<?php echo url('/admin/product', array_merge(request()->get(), ['page' => $currentPage - 1])); ?>">上一页</a>
                </li>
                <?php if($lastPage <= 7): $__FOR_START_1707024528__=1;$__FOR_END_1707024528__=$lastPage + 1;for($i=$__FOR_START_1707024528__;$i < $__FOR_END_1707024528__;$i+=1){ ?>
                <li class="page-item <?php if($currentPage == $i): ?>active<?php endif; ?>">
                    <a class="page-link" href="<?php echo url('/admin/product', array_merge(request()->get(), ['page' => $i])); ?>"><?php echo htmlentities((string) $i); ?></a>
                </li>
                <?php } else: if($currentPage <= 4): $__FOR_START_1107684517__=1;$__FOR_END_1107684517__=6;for($i=$__FOR_START_1107684517__;$i < $__FOR_END_1107684517__;$i+=1){ ?>
                <li class="page-item <?php if($currentPage == $i): ?>active<?php endif; ?>">
                    <a class="page-link" href="<?php echo url('/admin/product', array_merge(request()->get(), ['page' => $i])); ?>"><?php echo htmlentities((string) $i); ?></a>
                </li>
                <?php } ?>
                ...
                <li class="page-item">
                    <a class="page-link" href="<?php echo url('/admin/product', array_merge(request()->get(), ['page' => $lastPage])); ?>"><?php echo htmlentities((string) $lastPage); ?></a>
                </li>
                <?php elseif($currentPage >= $lastPage - 3): ?>
                <li class="page-item">
                    <a class="page-link" href="<?php echo url('/admin/product', array_merge(request()->get(), ['page' => 1])); ?>">1</a>
                </li>
                <li class="page-item disabled"><span class="page-link">...</span></li>
                <?php $__FOR_START_1941747560__=$lastPage - 4;$__FOR_END_1941747560__=$lastPage + 1;for($i=$__FOR_START_1941747560__;$i < $__FOR_END_1941747560__;$i+=1){ ?>
                <li class="page-item <?php if($currentPage == $i): ?>active<?php endif; ?>">
                    <a class="page-link" href="<?php echo url('/admin/product', array_merge(request()->get(), ['page' => $i])); ?>"><?php echo htmlentities((string) $i); ?></a>
                </li>
                <?php } else: ?>
                ...
                <li class="page-item disabled"><span class="page-link">...</span></li>
                <?php $__FOR_START_64707894__=$currentPage - 1;$__FOR_END_64707894__=$currentPage + 2;for($i=$__FOR_START_64707894__;$i < $__FOR_END_64707894__;$i+=1){ ?>
                <li class="page-item <?php if($currentPage == $i): ?>active<?php endif; ?>">
                    <a class="page-link" href="<?php echo url('/admin/product', array_merge(request()->get(), ['page' => $i])); ?>"><?php echo htmlentities((string) $i); ?></a>
                </li>
                <?php } ?>
                ...
                <li class="page-item <?php if($currentPage >= $lastPage): ?>disabled<?php endif; ?>">
                    <a class="page-link" href="<?php echo url('/admin/product', array_merge(request()->get(), ['page' => $currentPage + 1])); ?>">下一页</a>
                </li>
            </ul>
            </nav>
        </div>
        ...
        <div class="modal-content">
            ...
            <form id="editForm">
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label class="form-label">产品详情 <span class="text-danger">*</span></label>
                        <textarea class="form-control" name="content" id="editContent" rows="6" required></textarea>
                    </div>
                    <div class="row">
...
function openAdd() {
    $('#modalTitle').text('新增产品');
    $('#editForm')[0].reset();
    $('#editId').val('');
}

function openEdit(id) {
    $('#modalTitle').text('编辑产品');
    $.get('/admin/product/edit', {id: id}, function(res) {
        if (res.code === 1) {
            var d = res.data;
            $('#editId').val(d.id);
            $('#editName').val(d.name);
...
$('#editForm').on('submit', function(e) {
    e.preventDefault();
    var id = $('#editId').val();
    var url = id ? '/admin/product/update' : '/admin/product/save';
