<?php /*a:2:{s:101:"/Users/ding/Documents/GSB 3/4.15/label-00896-GSB-0415/frontend-admin/app/view/admin/config/index.html";i:1776232727;s:109:"/Users/ding/Documents/GSB 3/4.15/label-00896-GSB-0415/frontend-admin/app/view/admin/../admin/layout/base.html";i:1776232727;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
    ...
    <title>系统配置 - <?php echo htmlentities((string) $site_name); ?></title>
...
<div class="config-card">
    <div class="config-tabs">
        <a href="/admin/config?group=basic" class="tab-item <?php if($group == 'basic'): ?>active<?php endif; ?>">基本设置</a>
        <a href="/admin/config?group=contact" class="tab-item <?php if($group == 'contact'): ?>active<?php endif; ?>">联系信息</a>
    </div>

    <form id="configForm" class="config-form">
        <input type="hidden" name="group" value="<?php echo htmlentities((string) $group); ?>">

        <?php if(is_array($configs) || $configs instanceof \think\Collection || $configs instanceof \think\Paginator): $i = 0; $__LIST__ = $configs;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?>
        <div class="form-group">
            <label class="form-label"><?php echo htmlentities((string) $item['description']); ?></label>
            <?php switch($item['type']): case "'textarea'": ?>
            <textarea class="form-control" name="config[<?php echo htmlentities((string) $item['key_name']); ?>]" rows="3" placeholder="请输入<?php echo htmlentities((string) $item['description']); ?>"><?php echo htmlentities((string) $item['value']); ?></textarea>
            <?php break; case "'image'": ?>
            <div class="input-group">
                <input type="text" class="form-control" name="config[<?php echo htmlentities((string) $item['key_name']); ?>]" id="config_<?php echo htmlentities((string) $item['key_name']); ?>" value="<?php echo htmlentities((string) $item['value']); ?>" placeholder="请输入图片URL或上传">
                <button type="button" class="btn btn-outline-primary" onclick="uploadConfigImage('<?php echo htmlentities((string) $item['key_name']); ?>')"><i class="bi bi-upload me-1"></i>上传</button>
            </div>
            <?php if($item['value']): ?>
            <div class="mt-2">
                <img src="<?php echo htmlentities((string) $item['value']); ?>" class="img-thumbnail" style="max-width:120px;max-height:80px;" id="preview_<?php echo htmlentities((string) $item['key_name']); ?>">
            </div>
            <?php endif; break; default: ?>
            <input type="text" class="form-control" name="config[<?php echo htmlentities((string) $item['key_name']); ?>]" value="<?php echo htmlentities((string) $item['value']); ?>" placeholder="请输入<?php echo htmlentities((string) $item['description']); ?>">
            <?php endswitch; ?>
        </div>
        <?php endforeach; endif; else: echo "" ;endif; if(empty($configs) || (($configs instanceof \think\Collection || $configs instanceof \think\Paginator ) && $configs->isEmpty())): ?>
